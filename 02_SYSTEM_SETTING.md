...
Название машины хранится в /etc/hostname. Под старым пользователем выполним команду:

``sudo hostname user-1``

Таким образом мы сменили имя машины на ``user-1``. Проверим, что изменения применились:

``cat /etc/hostname``

Если изменения в файл не попали, вносим их ручками:

``nano /etc/hostname``

Так же проверим /etc/hosts:
...
И запишем туда свой hostname.

Сохраняем файл и перезагружаемся по ``sudo reboot``. При следующем входе вместо "user@ubuntu01" нас встретит "user@user-1".
