...
Смотрим и редактируем список серверов времени:

``sudo nano /etc/ntp.conf``

Добавим в конец файла дополнительные сервера синхронизации времени:
...
Отредактируем демона ntp:

``sudo nano /etc/systemd/timesyncd.conf``

И вставим в секцию [Time] адрес любого из вышеперечисленных серверов времени. Я выберу ntp1.imvp.ru. Не забываем раскомментировать строку:

![установка ubuntu](media/system_settings/step_07.png)

Сохраняем, закрываем, перезагружаем сервис ntp:

``sudo service ntp restart``

Проверим, что сервис нормально запустился:

``sudo service ntp status``

Выйдем по ``control+C`` и ещё раз проверим время при помощи ``timedatectl``. 