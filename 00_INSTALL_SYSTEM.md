...
Предполагается, что пользоваться редактором nano читатель данного гайда уже умеет. Так же есть vim и другие альтернативы nano.
...
Теперь мы можем выключать виртуальную машину командой ``sudo shutdown now`` и закреплять достигнутый результат, сохранив снимок состояния системы, сделав экспорт и загрузив результаты нашего труда, например, в облако. Об этом [следующий гайд](https://github.com/codesshaman/sber_devops_d01_linux-master/blob/main/01_EXPORT_AND_SAVE.md "Сохранение и экспорт").